...
def action_event_binding_runner_factory(action: Action, event_name: str, to_call=None,
    ...
    if isinstance(action, InitializableAction):
        return InitializableActionEventBindingRunner(action, event_name, to_call, fail_fast=fail_fast)
    return ActionEventBindingRunner(action, event_name, to_call, fail_fast=fail_fast)
